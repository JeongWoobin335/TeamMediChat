...
1. **datasource**: 
   ...
   - "MEDICINE_INFO": 약품 정보, 최신 약품, 신약, 약품 비교 등 정보 요청
...
- "2024년", "2023년", "새로", "최신", "새로운", "경험담", "후기" 같은 키워드가 있으면 반드시 MEDICINE_INFO로 분류하세요
- 최신 정보나 사용자 경험을 요청하는 질문은 MEDICINE_INFO로 분류하세요
...
def route_question_node(state: QAState) -> QAState:
    ...
    state["routing_decision"] = (
        "recommend" if result.datasource == "MEDICINE_RECOMMENDATION" else "search"
    )
    return state
