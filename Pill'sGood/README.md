# TeamMediChat - 의약품 사용 가능성 판단 시스템

## 개요

TeamMediChat은 실시간 대화를 통해 사용자의 약품 사용에 대한 질의를 판단하는 AI 기반 시스템입니다. LangGraph를 활용하여 복잡한 의료 질문을 단계별로 처리하고, 다양한 데이터 소스에서 정확한 정보를 제공합니다.
...
- **route_question_node**: 질문 유형에 따른 처리 경로 결정 (사용 가능성 판단/정보 검색/SNS 검색)
...
### 4. 특화 노드
- **medicine_usage_check_node**: 약품 사용 가능성 판단
- **ocr_image_node**:  모든 약품 형태 이미지에서 약품명 추출 
...
- **OCR 이미지 요청**: 모든 약품 형태 이미지에서 약품명 텍스트 추출 후 DB데이터와 연결하여 사용 가능성 판단
- **약품 사용 가능성 판단**: 특정 약품의 특정 상황에서의 사용 가능성
